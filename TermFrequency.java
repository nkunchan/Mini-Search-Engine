...
public class TermFrequency extends Configured implements Tool {
	...
	public static class Map extends
		...
		public void map(LongWritable offset, Text lineText, Context context)
			...
			for (String word : WORD_BOUNDARY.split(line)) {
				...
				// combine word and file name using delimiter "#####" and send to reducer along with value 1
	...
	public static class Reduce extends
		...
		public void reduce(Text word, Iterable<IntWritable> counts,
			...
			int sum = 0;
			//get the count for each word and sum it up to get the total no of occurances
			for (IntWritable count : counts) {
				sum += count.get();
			}
			double temp;
			// calculate term frequency using logarithmic equation
			if (sum > 0)
				temp = (1 + Math.log10(sum));
			else
				temp = 0;
			// print the word along with term frequency
